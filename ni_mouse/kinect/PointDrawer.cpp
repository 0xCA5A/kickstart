	...
	m_nHistorySize(nHistory), m_DepthGenerator(depthGenerator), m_bDrawDM(false), m_bFrameID(false)
{
	...
	mouseMoverPtr = new MouseMover();
	
	if(!mouseMoverPtr->setupUinputDevice())
	{
		printf("[%s] setup of uinput device failed...\n", __func__);
	}
...
void XnVPointDrawer::OnPointUpdate(const XnVHandPointContext* cxt)
{
	...
	{
	    //printf("[%s] update: %0d\n", __func__, ++cntr);
	    mouseMoverPtr->moveCursor(ptProjective.X, ptProjective.Y);
	}

 	if (cxt->nID > 1)
	{
	    //printf("[%s] update: %0d\n", __func__, ++cntr);
	    mouseMoverPtr->leftClickChecker(ptProjective.Y);
	}
